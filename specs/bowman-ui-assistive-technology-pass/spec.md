...
([validated by](../../tests/check-at-pass.test.ts#L173)), so `npm publish` is
blocked until someone listens, while `--structure` passes with a warning
([validated by](../../tests/check-at-pass.test.ts#L163)) so no pull request is
...
([validated by](../../examples/chat-demo/tests/chat-demo.spec.ts#L107)). A
second test counts the distinct growth steps and the span they cover: at least
20 steps over at least 3 seconds
([validated by](../../examples/chat-demo/tests/chat-demo.spec.ts#L136)). That
...
([validated by](../../tests/check-at-pass.test.ts#L270),
[without a reason](../../tests/check-at-pass.test.ts#L292),
[with one](../../tests/check-at-pass.test.ts#L310)).
...
1. **The record's front matter is the machine-readable part.** Row verdicts
   ...
   ([validated by](../../tests/check-at-pass.test.ts#L192)); a `commit` that is
   not 40 hex fails
   ([validated by](../../tests/check-at-pass.test.ts#L204)); a row missing a
   verdict on either stack fails, naming the row and the stack
   ([validated by](../../tests/check-at-pass.test.ts#L213)). The front-matter
   ...
   ([validated by](../../tests/check-at-pass.test.ts#L526)).
2. **Every `fail` has an owner.** A `fail` row naming neither a `fixing-issue`
   nor an `accepted-by` fails the gate
   ([validated by](../../tests/check-at-pass.test.ts#L231)); one naming a
   fixing issue passes
   ([validated by](../../tests/check-at-pass.test.ts#L251)). Fixing what a row
...
3. **A waiver is temporary by construction.** `waived` without both `waived-by`
   and `expires` fails
   ([validated by](../../tests/check-at-pass.test.ts#L329)); an expired waiver
   fails in both modes, with the same exit code as a stale record
   ([validated by](../../tests/check-at-pass.test.ts#L349)); an unexpired one
   passes ([validated by](../../tests/check-at-pass.test.ts#L373)). A first
...
4. **`covers` makes the record perishable.** `--freshness` takes the last
   ...
   ([validated by](../../tests/check-at-pass.test.ts#L407)); a covered path no
   commit touches fails too
   ([validated by](../../tests/check-at-pass.test.ts#L430)). Editing
...
5. **No record blocks the release, not the pull request.** `--structure`
   ...
   ([validated by](../../tests/check-at-pass.test.ts#L163)) and `--freshness`
   fails ([validated by](../../tests/check-at-pass.test.ts#L173)). The
...
6. **No placeholder survives.** `TBD`, `TODO`, `FIXME`, `XXX` and
   ...
   ([validated by](../../tests/check-at-pass.test.ts#L392)), which is what
...
7. **The newest record by filename is the one validated**
   ([validated by](../../tests/check-at-pass.test.ts#L445)). ISO dates sort
...
8. **Exit codes follow the house.** 1 lists every violation on stderr; 2 is a
   ...
   ([validated by](../../tests/check-at-pass.test.ts#L535)) or an unknown one
   ([validated by](../../tests/check-at-pass.test.ts#L542)).
...
`Test Runner`
([validated by](../../tests/check-at-pass.test.ts#L182)).
...
- **KU-21, who runs the pass.** No transcript names anyone for this role.
  ...
  ([validated by](../../tests/check-at-pass.test.ts#L192)), so whoever runs the
