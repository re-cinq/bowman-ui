...
inside `ChatMessage`. No file in `discovery` changes.
...
`scores` gone ([validated by](../../tests/ChatMessage.test.tsx#L508)).
...
([validated by](../../tests/types/chat-message-type-assertions.tsx#L53),
...
1. **Two roles only, no silent drop.** `entry` is
   ...
   ([validated by](../../tests/types/chat-message-type-assertions.tsx#L44)).
2. **Arrow-key feedback is opt-in; `Cmd/Ctrl+C` stays on.** `arrowKeyFeedback`
   ...
   ([validated by](../../tests/ChatMessage.test.tsx#L173)); enabled, the
   shortcuts work as before
   ([validated by](../../tests/ChatMessage.test.tsx#L183),
   [L208](../../tests/ChatMessage.test.tsx#L208)). Copy stays unconditional
   with its selection guard
   ([validated by](../../tests/ChatMessage.test.tsx#L124),
   [L148](../../tests/ChatMessage.test.tsx#L148)). The default labels follow:
...
3. **`showFeedback` gates the keyboard path too.** Arrow handling fires only
   ...
   ([validated by](../../tests/ChatMessage.test.tsx#L233)).
4. **The render-phase latch stays, keyed per `entry.id`.** Once content or
   ...
   ([validated by](../../tests/ChatMessage.test.tsx#L311)); a rerender with a
   different id, streaming and empty, shows it again - removing the id reset
   fails the test ([validated by](../../tests/ChatMessage.test.tsx#L331)).
...
| #   | Adaptation                                                                                                                                                                                               | Reason                                                                                          |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| a   | The two arrow-key feedback tests pass `arrowKeyFeedback` and assert the parenthetical-free thumb labels ([L183](../../tests/ChatMessage.test.tsx#L183), [L208](../../tests/ChatMessage.test.tsx#L208))   | Decision 2: the shortcuts are opt-in and the default labels no longer mention them              |
| b   | New default-off test: `ArrowUp` with default props calls `onFeedback` zero times and does not `preventDefault` (asserted via `fireEvent`'s return value) ([L173](../../tests/ChatMessage.test.tsx#L173)) | Decision 2: the library default must not steal a keyboard user's scroll keys                    |
| c   | New test: `arrowKeyFeedback` + `showFeedback={false}` fires nothing and renders no thumbs ([L233](../../tests/ChatMessage.test.tsx#L233))                                                                | Decision 3: in the source, the handler never read `showFeedback`                                |
| d   | The latch test gains a second direction: a new `entry.id`, streaming and empty, shows the indicator again ([L331](../../tests/ChatMessage.test.tsx#L331))                                                | Decision 4: the source's module-lifetime ref masked the bug because the app keys its list by id |
| e   | The streaming no-key-handling test passes `arrowKeyFeedback` ([L272](../../tests/ChatMessage.test.tsx#L272))                                                                                             | It must still exercise the `isStreaming` gate now that arrows are off by default                |
...
([validated by](../../tests/ChatMessage.test.tsx#L420)), because 019 shipped
every package animation class under the `bowman-` prefix (its spec: names
"cannot collide with a consumer's own `animate-*` utilities"). That is the
same rename the confirmation spans (`bowman-fade-in`,
[validated by](../../tests/ChatMessage.test.tsx#L472)) and the indicator dots
(`bowman-fade-dot`,
[validated by](../../tests/InlineThinkingIndicator.test.tsx#L18)) received.
The tool-status row keeps `animate-spin` unchanged
([validated by](../../tests/ChatMessage.test.tsx#L299)): that is a Tailwind
...
- Markdown renders through 019's `markdownComponents` map with `remarkGfm`;
  ...
  [L535](../../tests/ChatMessage.test.tsx#L535)).
- Raw HTML in `entry.content` stays escaped text; `rehype-raw` appears in no
  ...
  [L523](../../tests/ChatMessage.test.tsx#L523),
...
- The avatar circle takes `assistantAvatar` in place of the hardcoded logo and
  ...
  ([validated by](../../tests/ChatMessage.test.tsx#L400),
  [L412](../../tests/ChatMessage.test.tsx#L412)).
- `footer` collapses the two dev-harness slots into one `ReactNode` rendered
  last in the message column, streaming or not
  ([validated by](../../tests/ChatMessage.test.tsx#L440),
  [L450](../../tests/ChatMessage.test.tsx#L450),
  [L462](../../tests/ChatMessage.test.tsx#L462)).
- The four icons come from 020's set via relative `.js` imports; no `@clerk`,
  `swr`, `next-intl`, `next/`, `@discovery` or `@/` import survives
  ([validated by](../../tests/ChatMessage.test.tsx#L512)); `"Discovery"`
...
- Both files carry `"use client"` as the first statement of their `dist/`
  ...
  ([validated by](../../tests/chat-message-dist.test.ts#L34)) and
...
- `navigator.clipboard` is optional-chained: an insecure-context browser is a
  ...
  ([validated by](../../tests/ChatMessage.test.tsx#L160)). The 2000ms
...
- **`InlineThinkingIndicator` takes `labels`, not a bare `label` string.** The
  ...
  [partition](../../tests/labelled-exports.test.tsx#L27)). `ChatMessage`
...
- **Sentinel harness content.** 022's sentinel test covers both components;
  ...
  ([validated by](../../tests/labelled-exports.test.tsx#L218)).
- **GDPR enforcement is a source grep plus a suite-wide console trap.**
  ...
  ([validated by](../../tests/ChatMessage.test.tsx#L529)), and
...
- **`react-markdown` and `remark-gfm` become runtime dependencies** at the
  ...
  devDependencies ([validated by](../../tests/chat-message-dist.test.ts#L88));
  019's zero-runtime-deps claim carries a supersession note in its spec.
