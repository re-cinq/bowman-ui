...
- The action row under a committed assistant entry is `opacity-0` until the pointer
  ...
  ([validated by an assistant entry's action row is invisible at rest and revealed by hover or by focus](../../examples/chat-demo/tests/chat-demo.spec.ts#L489)).
...
- Both files carry `"use client"` as the first statement of their `dist/`
  ...
  ([validated by each built chat message component opens with "use client"; as its first statement](../../tests/chat-message-dist.test.ts#L12)).
...
- **Sentinel harness content.** 022's sentinel test covers both components;
  ...
  ([validated by ChatMessage's sentinel labels cover every defaultChatMessageLabels key](../../tests/labelled-exports.test.tsx#L555),
...
- **`react-markdown` and `remark-gfm` are runtime dependencies** at
  ...
  in its spec ([validated by react-markdown 10 and remark-gfm 4 are runtime dependencies and no longer devDependencies](../../tests/chat-message-dist.test.ts#L56)).
