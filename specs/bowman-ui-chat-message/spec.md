...
- The action row under a committed assistant entry is `opacity-0` until the pointer
  ...
  ([validated by an assistant entry's action row is invisible at rest and revealed by hover or by focus](../../examples/chat-demo/tests/chat-demo.spec.ts#L510)).
...
- Long unbroken strings wrap: the user bubble and the assistant prose
  ...
  [browser](../../examples/chat-demo/tests/chat-demo.spec.ts#L389)).
