...
Issue: re-cinq/bowman-ui#2 (`non-trailing spec test links; commit the placement validator and
gate it in CI`), which subsumes re-cinq/bowman-ui#7 (`broken test links in spec.md`). The
enforcement decision lives in docs/design-notes.md § Lint guardrails decision 10; this file
carries the placement rule, the script's contract, and the per-behaviour pins.
...
- A citation mid-way through a list item is misplaced, and each one is reported separately
  ...
  ([validated by](../../tests/check-spec-links.test.ts#L48)).
...
`--experimental-strip-types`, which is why the repo's Node floor of 22 matters to it.
...
- A relative path means the same spec from any working directory
  ([validated by](../../tests/check-spec-links.test.ts#L133)).
- An absolute path is accepted and reported root-relative
  ([validated by](../../tests/check-spec-links.test.ts#L137)).
- The usage line states both facts
  ([validated by](../../tests/check-spec-links.test.ts#L120)).
