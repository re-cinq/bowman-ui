...
([validated by](../../tests/ChatComposer.test.tsx#L382)). External writes go
...
([validated by](../../tests/ChatComposer.test.tsx#L194),
...
- The trimmed draft is submitted once and the composer clears itself: value
  ...
  ([validated by](../../tests/ChatComposer.test.tsx#L34)).
- `"  Ja  "` submits as `"Ja"` - no length floor, rejecting `the-expert-ui`'s
  3-character floor;
  `"   "` leaves send disabled and `onSubmit` uncalled, also under `Enter`
  ([validated by](../../tests/ChatComposer.test.tsx#L48),
  [L60](../../tests/ChatComposer.test.tsx#L60),
  [L94](../../tests/ChatComposer.test.tsx#L94)).

## The keyboard

- `Enter` submits and preventDefaults;
  `Shift+Enter` inserts a newline and leaves the draft
  ([validated by](../../tests/ChatComposer.test.tsx#L82),
  [L133](../../tests/ChatComposer.test.tsx#L133),
  [L105](../../tests/ChatComposer.test.tsx#L105)).
- **The IME guard is the fix this component adds.** `Enter` with
  ...
  [validated by](../../tests/ChatComposer.test.tsx#L117)).
...
[L217](../../tests/ChatComposer.test.tsx#L217)). `focus()` makes the
textarea `document.activeElement`; `autoFocus` does
the same on mount and defaults to false
([validated by](../../tests/ChatComposer.test.tsx#L227),
[L237](../../tests/ChatComposer.test.tsx#L237)).

## Auto-resize

`height = auto` then `min(scrollHeight, maxHeightPx)`, with `200` as the
`maxHeightPx` default, `resize-none` with scroll past the
cap ([validated by](../../tests/ChatComposer.test.tsx#L249)). jsdom performs no layout and reports `scrollHeight` 0, so the tests stub
...
([validated by](../../tests/ChatComposer.test.tsx#L249),
[L258](../../tests/ChatComposer.test.tsx#L258)). A passing test here
...
so no button ships without a slot to fill it) ([validated by](../../tests/ChatComposer.test.tsx#L387)).
With no `attachSlot`, send is the only button; a supplied slot
renders left of send
([validated by](../../tests/ChatComposer.test.tsx#L269),
[L275](../../tests/ChatComposer.test.tsx#L275)). The wrapper is a
...
([validated by](../../tests/ChatComposer.test.tsx#L295),
[L303](../../tests/ChatComposer.test.tsx#L303)).
...
([validated by](../../tests/ChatComposer.test.tsx#L336)). The textarea answers to the resolved
...
([validated by](../../tests/ChatComposer.test.tsx#L321), defaults
[L336](../../tests/ChatComposer.test.tsx#L336),
[L344](../../tests/ChatComposer.test.tsx#L344)).
...
covers it ([validated by](../../tests/labelled-exports.test.tsx#L532)).
...
default
([validated by](../../tests/ChatComposer.test.tsx#L336)).
...
([validated by](../../tests/ChatComposer.test.tsx#L400)). There is no draft persistence
and no autosave: an unsent support question does not survive on the
customer's device ([validated by](../../tests/ChatComposer.test.tsx#L400)).

## Source purity and the build

No `@clerk`, `swr`, `next-intl`, `next/`, `@/` or
`lucide-react` import, and every relative import ends in `.js`
([validated by](../../tests/ChatComposer.test.tsx#L408)).
