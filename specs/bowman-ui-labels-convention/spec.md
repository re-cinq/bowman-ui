...
- `src/labels.ts` -
  ...
  catalogue lookup produces ([validated by { copy: "Copiar" } replaces copy and keeps every other default](../../tests/resolveLabels.test.ts#L17),
  [validated by an undefined overrides argument returns the defaults](../../tests/resolveLabels.test.ts#L24),
  [validated by { copy: undefined } - a consumer's missed catalogue lookup - returns "Copy message", not undefined](../../tests/resolveLabels.test.ts#L28)).
- A function-valued label - the convention's interpolation form - overrides like any other key
  ([validated by a function-valued label overrides like any other key, pinning the interpolation form](../../tests/resolveLabels.test.ts#L32)).
- Neither argument is mutated and `Object.freeze`d defaults do not throw
  ([validated by mutates neither argument](../../tests/resolveLabels.test.ts#L41),
...
- `src/components/ErrorBoundary.tsx` retrofitted onto `resolveLabels` as the worked example:
  ...
  `"."` exports entry ([validated by tsc accepts labels-type-assertions.tsx against dist via the '.' exports entry](../../tests/labels-dist.test.ts#L3)).
- The three translated labels render with no English remaining; no `labels` prop renders the three
  021 English defaults unchanged ([validated by labels override the defaults per key and no English remains](../../tests/ErrorBoundary.test.tsx#L104),
...
- `eslint.config.mjs` gains a labels entry (core ESLint only): `no-restricted-syntax` selectors
  ...
  `next-intl` ([validated by `<span>Copied!</span> fails with the hardcoded-JSX-text message`](../../tests/eslint-labels.test.ts#L31),
  [validated by aria-label="Copy message" fails with the hardcoded-assistive-string message](../../tests/eslint-labels.test.ts#L97),
  [validated by `a strings?: Record<string, string> prop fails with the labels-prop-name message`](../../tests/eslint-labels.test.ts#L106),
  [validated by importing next-intl fails with the no-i18n-runtime message](../../tests/eslint-labels.test.ts#L124)).
...
- Re-pinned prior behaviour (AC 39): `<LoadingIcon ariaLabel="Cargando" />` renders
  ...
  ([validated by ariaLabel="Cargando" renders aria-label="Cargando" and "Loading" appears nowhere](../../tests/icons.test.tsx#L266),
...
- **Behaviour change: explicit-`undefined` overrides.** 021's
  ...
  ([validated by an explicit undefined label - a consumer's missed catalogue lookup - falls back to the English default](../../tests/ErrorBoundary.test.tsx#L126)).
- **Partition is over value exports.** The partition test statically parses `export { ... }`
  ...
  members ([validated by labelsProp, stringPropOnly and noStrings together are exactly src/index.ts's value exports](../../tests/labelled-exports.test.tsx#L158)).
...
- **Fixture-scope deviation.** The labels lint entry's `files` glob covers
  ...
  ([validated by `<span>Copied!</span> fails with the hardcoded-JSX-text message`](../../tests/eslint-labels.test.ts#L31)).
- **Placeholder deleted.** `src/Placeholder.tsx`, `tests/Placeholder.test.tsx` and the barrel
  ...
  file ([validated by emits "use client"; as the first statement of dist/hooks/useDebounce.js](../../tests/build-contract.test.ts#L12)).
