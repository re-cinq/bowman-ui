...
[validated by is frozen and equals { suggestedPrompts: "Suggested prompts" }](../../tests/PromptChips.test.tsx#L127), [validated by defaultSearchFieldLabels is frozen and equals { searchInput: "Search", searchPlaceholder: "Search..." }](../../tests/SearchField.test.tsx#L106)).
Every value export lands in one bucket of `tests/labelled-exports.test.tsx`
([validated by labelsProp, stringPropOnly and noStrings together are exactly src/index.ts's value exports](../../tests/labelled-exports.test.tsx#L158)). The `labelsProp` bucket holds
...
closed members ([validated by labelsProp, stringPropOnly and noStrings together are exactly src/index.ts's value exports](../../tests/labelled-exports.test.tsx#L158)).
...
- Renders one `<button>` with `type` defaulting to `"button"`, the `children` as its accessible
  name, and the `ref` forwarded to that element ([validated by `renders one <button type="button"> whose accessible name is the children "Ny samtale"`](../../tests/Button.test.tsx#L26),
  [validated by type="submit" renders type="submit"](../../tests/Button.test.tsx#L33), [validated by `the ref reaches the <button> element`](../../tests/Button.test.tsx#L43)).
- `variant` picks the look: `primary` is the composer send button's accent fill (the
  ...
  [validated by variant="primary" carries the --bowman-accent background and hover, --bowman-text-on-accent text and no border-slate-200](../../tests/Button.test.tsx#L96), [validated by variant="ghost" reads --bowman-text-secondary, its hover reads --bowman-text-strong, beside --bowman-surface-hover with neither a border token nor the --bowman-accent background](../../tests/Button.test.tsx#L113),
  [validated by every variant keeps focus:ring-2 ring-offset-2 beside the --bowman-focus-ring and --bowman-ring-offset colours and carries the disabled pair](../../tests/Button.test.tsx#L123)).
- `ghost`'s hover text and the `SearchField` input read `--bowman-text-strong`
  ...
  ([validated by variant="ghost" reads --bowman-text-secondary, its hover reads --bowman-text-strong, beside --bowman-surface-hover with neither a border token nor the --bowman-accent background](../../tests/Button.test.tsx#L110), [validated by the input reads the strong text token, so a consumer recolours the query text with the theme](../../tests/SearchField.test.tsx#L123)).
...
- `icon`, when given, renders before the text with `h-4 w-4` at `md` and `h-3.5 w-3.5` at `sm`;
  ...
  `ariaLabel` ([validated by `icon={PlusIcon} at md renders an aria-hidden <svg class="h-4 w-4"> before the text "Ny samtale"`](../../tests/Button.test.tsx#L163),
  [L183](../../tests/Button.test.tsx#L183),
  [validated by with icon omitted no svg renders and the button holds only the text](../../tests/Button.test.tsx#L193)).
- `onClick` receives the click event; `disabled` renders the native attribute and the click never
  fires ([validated by clicking "Ny samtale" calls onClick once with the click event](../../tests/Button.test.tsx#L57), [validated by disabled renders the native attribute and the click never reaches onClick](../../tests/Button.test.tsx#L69)).
...
- Renders one `<button>` named by `labels.accessibleName` through `aria-label`, containing only
  ...
  `Button` ([validated by `renders one <button type="button"> named "Tilføj 4711" through aria-label with empty text content`](../../tests/IconButton.test.tsx#L27),
  [validated by `the button contains only one aria-hidden <svg class="h-4 w-4"> at md`](../../tests/IconButton.test.tsx#L50),
  [validated by with size omitted the button carries the square md padding p-2 and no text scale](../../tests/IconButton.test.tsx#L124), [L133](../../tests/IconButton.test.tsx#L133),
  [validated by with variant omitted the button is secondary: border beside --bowman-border, --bowman-surface and --bowman-text-body, no --bowman-accent background](../../tests/IconButton.test.tsx#L83), [validated by variant="primary" carries the --bowman-accent background and hover, --bowman-text-on-accent text and no border-slate-200](../../tests/IconButton.test.tsx#L96),
  [validated by variant="ghost" reads --bowman-text-secondary and --bowman-surface-hover and no border class at all](../../tests/IconButton.test.tsx#L99), [validated by focus:ring-2 ring-offset-2 stay beside the --bowman-focus-ring and --bowman-ring-offset colours and the disabled pair is present](../../tests/IconButton.test.tsx#L112)).
- `IconButton` shares `Button`'s `type` default and `"submit"` option, its `onClick` event and its
  `disabled` behaviour ([validated by type="submit" renders type="submit"](../../tests/IconButton.test.tsx#L36),
  [validated by `renders one <button type="button"> named "Tilføj 4711" through aria-label with empty text content`](../../tests/IconButton.test.tsx#L27), [validated by clicking "Tilføj 4711" calls onClick once with the click event](../../tests/IconButton.test.tsx#L61),
  [validated by disabled renders the native attribute and the click never reaches onClick](../../tests/IconButton.test.tsx#L69)).
...
- Renders a `<ul role="list">` named by `suggestedPrompts`, one `<li>` per prompt holding a
  ...
  ([validated by `renders a <ul> with role="list" named "Suggested prompts" by default`](../../tests/PromptChips.test.tsx#L21),
  [validated by labels={{ suggestedPrompts: "Forslag" }} names the list "Forslag" and "Suggested prompts" appears nowhere](../../tests/PromptChips.test.tsx#L31),
  [validated by labels={{ suggestedPrompts: undefined }} falls back to "Suggested prompts"](../../tests/PromptChips.test.tsx#L44), [validated by `three prompts render three type="button" chips named by the prompt texts, in order, one per <li>`](../../tests/PromptChips.test.tsx#L65),
...
- Clicking a chip calls `onPick` once with exactly that prompt's text
  ([validated by clicking "Hvor er min booking?" calls onPick once with exactly that string](../../tests/PromptChips.test.tsx#L82),
  [validated by clicking "Skift afrejsedato" leaves the other two prompts uncalled](../../tests/PromptChips.test.tsx#L93)).
- An empty `prompts` array renders nothing at all - no list, no accessible name
  ([validated by](../../tests/PromptChips.test.tsx#L56)).
- Two identical prompt strings render two chips; keys are index-qualified so React never
  collapses them ([validated by two identical prompts "Send faktura 4711" render two chips](../../tests/PromptChips.test.tsx#L103)).
...
- Renders a wrapper `<div class="relative">` holding a decorative `SearchIcon` (absolutely
  ...
  the `ref` forwarded to the input ([validated by `renders one <input type="search"> named "Search" with placeholder "Search..." by default`](../../tests/SearchField.test.tsx#L16),
...
- No clear button, no submit, no debounce: filtering as the user types is the consumer's, and
  `useDebounce` already ships for it ([validated by renders no button - no clear control and no submit](../../tests/SearchField.test.tsx#L76)).
...
- `Button`, `IconButton` and `SearchField` are `forwardRef` components (decision 4: no cleanup
  ...
  ([validated by `the ref reaches the <button> element`](../../tests/Button.test.tsx#L43), [validated by `the ref reaches the <button> element`](../../tests/IconButton.test.tsx#L42),
  [validated by `the forwarded ref reaches the <input>`](../../tests/SearchField.test.tsx#L54)).
...
1. **A third variant, `ghost`.** The issue proposes `primary | secondary`. Two controls need a
   ...
   vocabularies ([validated by variant="ghost" reads --bowman-text-secondary, its hover reads --bowman-text-strong, beside --bowman-surface-hover with neither a border token nor the --bowman-accent background](../../tests/Button.test.tsx#L113),
   [validated by variant="ghost" reads --bowman-text-secondary and --bowman-surface-hover and no border class at all](../../tests/IconButton.test.tsx#L99),
...
5. **`SearchField` reports the raw value.** A controlled input that trimmed on the way out would
   ...
   reflects ([validated by a change to " Hvor " calls onChange once with " Hvor " untrimmed](../../tests/SearchField.test.tsx#L31)).
