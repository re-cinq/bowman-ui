...
([validated by](../../tests/ThinkingTrace.test.tsx#L191)). It is the fourth and
...
([validated by](../../tests/ThinkingTrace.test.tsx#L191)).
...
([validated by](../../tests/ThinkingTrace.test.tsx#L137)), resolved per key by
the convention's `resolveLabels`
([validated by](../../tests/ThinkingTrace.test.tsx#L125)).
...
1. **Collapsed, and it never opens itself.** The content sits behind a native
   ...
   label ([validated by](../../tests/ThinkingTrace.test.tsx#L35)) and carries
   no word of the content as a preview
   ([validated by](../../tests/ThinkingTrace.test.tsx#L44)). Expanding reveals
   the full content string
   ([validated by](../../tests/ThinkingTrace.test.tsx#L56)). No prop and no
   entry state sets `open`: neither `isStreaming` value does
   ([validated by](../../tests/ThinkingTrace.test.tsx#L85),
   [resting](../../tests/ThinkingTrace.test.tsx#L95)), and the source assigns
   the attribute nowhere
   ([validated by](../../tests/ThinkingTrace.test.tsx#L174)).
2. **Plain text, never markdown.** The content renders as `whitespace-pre-wrap`
   ...
   document as text
   ([validated by](../../tests/ThinkingTrace.test.tsx#L66)); the source
   references no `dangerouslySetInnerHTML`, `react-markdown` or `remark-`
   ([validated by](../../tests/ThinkingTrace.test.tsx#L151)). HAL's parser
...
3. **The dots mark streaming, inside the summary.** While `entry.isStreaming`
   ...
   ([validated by](../../tests/ThinkingTrace.test.tsx#L85)); with it false no
   dot is in the document
   ([validated by](../../tests/ThinkingTrace.test.tsx#L95)).
4. **`reducedMotion` goes through `021`'s hook.** The prop is forwarded to
   ...
   ([validated by](../../tests/ThinkingTrace.test.tsx#L157)); true strips the
   `bowman-fade-dot` animation class from the three dots
   ([validated by](../../tests/ThinkingTrace.test.tsx#L104)) and false keeps the
   class with `024`'s staggered delays
   ([validated by](../../tests/ThinkingTrace.test.tsx#L115)). `ThinkingDots`
...
5. **It is not a message.** `ThinkingTraceProps` declares none of `onCopy`,
   ...
   ([validated by](../../tests/ThinkingTrace.test.tsx#L162)) - internal
...
([validated by](../../tests/types/chat-message-list-type-assertions.tsx#L67),
compiled by
...
([validated by](../../tests/ChatMessageList.test.tsx#L1395)) while the user and
assistant entries render unchanged
([validated by](../../tests/ChatMessageList.test.tsx#L1409)). With it true the
same array renders exactly one collapsed `ThinkingTrace` between them
([validated by](../../tests/ChatMessageList.test.tsx#L1430)). `reducedMotion`
forwards to the trace
([validated by](../../tests/ChatMessageList.test.tsx#L1469)), as does the
resolved `thinkingTrace` label
([validated by](../../tests/ChatMessageList.test.tsx#L1486)).
...
`aiDisclosure` band
([validated by](../../tests/ChatMessageList.test.tsx#L1456)).
...
([validated by](../../tests/ThinkingTrace.test.tsx#L168)), and rendering the
fixture entry expanded leaves `localStorage.length` at `0`
([validated by](../../tests/ThinkingTrace.test.tsx#L180)).
...
sentinel harness
([validated by](../../tests/labelled-exports.test.tsx#L399)), and its sentinel
labels cover every `defaultThinkingTraceLabels` key
([validated by](../../tests/labelled-exports.test.tsx#L565)).
