...
`src/styles.css` ([validated by dist/styles.css opens with one comment line per token, forty-four in all, each stating its default](../../tests/theming-tokens-dist.test.ts#L191)).
...
([validated by the token table in docs/design-notes.md § Theming lists the same forty-four names and fallbacks, in the declared order](../../tests/theming-tokens-dist.test.ts#L283)).
...
- The set of tokens read across `dist/theme/tokens.js` and `dist/styles.css` is exactly the
  ...
  ([validated by the tokens read in dist/theme/tokens.js and dist/styles.css are exactly the forty-four declared ones](../../tests/theming-tokens-dist.test.ts#L198)).
- CLAUDE.md invariant 13 quotes the count as a literal, and the same dist test reads that digit
  ...
  ([validated by CLAUDE.md invariant 13 states the same token count as the stylesheet's declaration block](../../tests/theming-tokens-dist.test.ts#L339)).
...
- Every `--bowman-*` occurrence outside the declaration block is a `var()` read carrying a
  non-empty fallback ([validated by](../../tests/theming-tokens-dist.test.ts#L204)).
- A token falls back to the same palette value at every site that reads it, and that value is
  ...
  ([validated by each token falls back to the same palette value at every site, matching the declared default](../../tests/theming-tokens-dist.test.ts#L218)).
- No bare `blue-` palette utility survives in any built component, the tokens module or the
  ...
  ([validated by no bare blue- palette utility survives in any built component, the tokens module or the stylesheet](../../tests/theming-tokens-dist.test.ts#L266)).
...
value ([validated by no built component carries a --bowman- literal; the class strings live only in dist/theme/tokens.js](../../tests/theming-tokens-dist.test.ts#L287),
[validated by `no dist file assigns a --bowman-* value: every occurrence outside the stylesheet's comment block is a read`](../../tests/theming-tokens-dist.test.ts#L298)).

- `dist/theme/tokens.js` carries no `"use client"` directive, and `dist/index.js` re-exports
  nothing from it ([validated by dist/theme/tokens.js carries no "use client" directive and dist/index.js re-exports nothing from it](../../tests/theming-tokens-dist.test.ts#L278)).
...
- `bowman-pulse-subtle`'s 50 % stop reads `--bowman-accent-glow` and `--bowman-pulse-outline`
  ...
  [validated by the keyframe's 50% stop reads --bowman-accent-glow and --bowman-pulse-outline while its zero stop stays literal](../../tests/theming-tokens-dist.test.ts#L235)).
...
utility fails ([validated by each neutral role constant is imported by exactly the built components recorded for it](../../tests/theming-tokens-dist.test.ts#L247)).
...
- The `ChatComposer` textarea, `SearchField` input, `ErrorBoundary` heading, active `AppSidebar`
  ...
  ([validated by each neutral role constant is imported by exactly the built components recorded for it](../../tests/theming-tokens-dist.test.ts#L247), [validated by the message body reads the strong text token, joining text-strong rather than text-body per decision 6](../../tests/ChatMessage.test.tsx#L511)).
- `ChatMessage`'s assistant name, the `ThinkingTrace` trace content and the `ToolActivity` tool
  ...
  ([validated by each neutral role constant is imported by exactly the built components recorded for it](../../tests/theming-tokens-dist.test.ts#L247),
...
- `ChatMessage`'s copy and thumb buttons read `--bowman-text-subtle` at rest and
  ...
  slate-400 ([validated by each neutral role constant is imported by exactly the built components recorded for it](../../tests/theming-tokens-dist.test.ts#L247),
...
- The twenty-one role fallbacks are declared in the stylesheet block and the design-notes table
  ...
  ([validated by dist/styles.css opens with one comment line per token, forty-four in all, each stating its default](../../tests/theming-tokens-dist.test.ts#L191),
  [validated by the token table in docs/design-notes.md § Theming lists the same forty-four names and fallbacks, in the declared order](../../tests/theming-tokens-dist.test.ts#L283)).
...
- The copied check mark and the selected thumbs-up read `--bowman-success` for their text, and the
  ...
  ([validated by each neutral role constant is imported by exactly the built components recorded for it](../../tests/theming-tokens-dist.test.ts#L247), [validated by the copied check and the selected thumbs-up read the success role, retiring the green literals](../../tests/ChatMessage.test.tsx#L545)).
...
4. **`--bowman-accent` falls back to blue-500 in light mode and blue-600 in dark, with the hover
   ...
   selector ([validated by each token falls back to the same palette value at every site, matching the declared default](../../tests/theming-tokens-dist.test.ts#L218)).
...
tarball via `npm run consumer` (50 passed across the chat, docs and theming suites, exit 0); the
...
([validated by the send button and the composer's surface resolve to the dark palette fallbacks](../../examples/chat-demo/tests/theming.spec.ts#L310)). Not measured in Chromium
