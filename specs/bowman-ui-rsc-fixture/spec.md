...
Issue: issue 100 (the RSC fixture build CONTRACT.md decision 1 calls
...
measured build banner reads `Next.js 16.3.3 (Turbopack)`. `typescript` is
...
([validated by](../../CONTRACT.md#L401)), and the same sentence appears in
...
pages differ in content (`AppSidebar` vs a bare `nav`) so neither is a copy
of the other.

No `"react-server"` condition is added to `exports`; CONTRACT.md records the
refusal and its reason ([validated by](../../CONTRACT.md#L393)).
...
([validated by](../../scripts/rsc-fixture.sh#L96)).
...
([validated by](../../.github/workflows/ci.yml#L114)) - the issue text
...
`node-version: "22"` ([validated by](../../.github/workflows/ci.yml#L119)),
...
([validated by](../../.github/workflows/ci.yml#L125),
[the red step](../../.github/workflows/ci.yml#L127)).
...
node_modules `find`
([validated by](../../.github/workflows/ci.yml#L139)). One honest caveat,
...
([validated by](../../.github/workflows/ci.yml#L71)), and the consumer
scan ([validated by](../../scripts/consumer-app.sh#L97)).
...
reachable the same way `consumer` is, through `package.json`.
...
- Next's build artifacts cannot leak into the gates: `.next/`,
  `next-env.d.ts`, `tsconfig.tsbuildinfo` and the fixture lockfile are
  git-ignored ([validated by](../../.gitignore#L7)), the generated paths are
...
- No file under `src/` changed in this PR (the fixture's own
  `examples/rsc-fixture/src/` is the issue's named path for fixture data
  and is not the library's `src/`) - verifiable with
  `git diff --name-only origin/main | grep '^src/'` returning nothing.
