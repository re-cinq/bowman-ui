...
- `.github/workflows/publish.yml` runs when a GitHub Release is published. A `release-tag` job first refuses a malformed or prerelease tag; a credential-free `verify` job then re-runs every gate (lint, typecheck, markdown safety, the coverage suite, the consumer and RSC tarball proofs) at the release's tag; only then does the `publish` job, holding `id-token: write` and nothing else, stamp the tag's version into `package.json` (`main` carries the placeholder `0.0.0`; the tag is the version), build, and `npm stage publish --access public --provenance`. The trusted publisher allows staged publishing only: the version, its dist-tag and its provenance reach the registry, and a maintainer approves it with 2FA on npmjs.com before it is installable (chosen 2026-09-10 on npm's recommendation).
