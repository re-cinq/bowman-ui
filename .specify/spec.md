...
3. **Composer** — `ChatComposer` is a text-input and submission surface that owns its own draft ([validated by typing "Hvor er min booking?" and clicking send calls onSubmit once with exactly that string, then the draft is "", the height is back to auto and send is disabled again](../tests/ChatComposer.test.tsx#L41))
...
5. **App Shell** — `AppShell` is the top-level layout frame; `AppSidebar` supplies the `aside`/`nav` landmarks and navigation map ([validated by `renders children inside <main id="main-content"> by default`](../tests/AppShell.test.tsx#L24), [AppSidebar](../tests/AppSidebar.test.tsx#L22))
...
| Telemetry           | None — the package emits none ([validated by `GDPR: the file calls no console.*, localStorage, sessionStorage, fetch, sendBeacon or analytics, and holds no draft persistence`](../tests/ChatComposer.test.tsx#L463))                                                                                                                                                                                                      |
...
3. **Controlled by Default** — Components prefer controlled props; the five shipped hooks are `useDebounce`, `useFocusTrap`, `useFocusGroups`, `useReducedMotion`, and `useSidebarState`. ([validated by npm pack --dry-run ships the five hooks and ErrorBoundary with their d.ts files](../tests/hooks-dist.test.ts#L41))
...
3. **Type Definitions Included** — `.d.ts` files are bundled for full TypeScript IDE support. ([validated by npm pack --dry-run ships the five hooks and ErrorBoundary with their d.ts files](../tests/hooks-dist.test.ts#L41))
...
5. **No Internal Implementation Details Exposed** — Private modules and helpers are not exported; only public contracts are. ([validated by keeps the mapping function out of the published surface - dist/index.d.ts has no such symbol](../tests/types/chat.test.ts#L237), [L43](../tests/public-api.test.ts#L43))
