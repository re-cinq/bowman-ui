...
| Routing             | Consumer                                                                                                                                                                                                                                                                                                                                                                                                                   |
| i18n/localization   | Consumer ([validated by no run of three Latin letters survives outside the sentinels for any labelsProp member](../tests/labelled-exports.test.tsx#L638))                                                                                                                                                                                                                                                                  |
...
3. **Controlled by Default** — Components prefer controlled props; the five shipped hooks are `useDebounce`, `useFocusTrap`, `useFocusGroups`, `useReducedMotion`, and `useSidebarState`. ([validated by npm pack --dry-run ships the five hooks and ErrorBoundary with their d.ts files](../tests/hooks-dist.test.ts#L23))
4. **Stylesheet Ships With the Package** — Components carry Tailwind utility class names; `./styles.css` supplies the four keyframes and markdown/sr-only rules Tailwind cannot generate, and the consumer's Tailwind v4 build scans `dist`. ([validated by exports gains "./styles.css" alongside the unchanged "." entry](../tests/styles.test.ts#L107), [source](../tests/tailwind-build.test.ts#L79))
...
2. **Stylesheet Subpath Export** — `./styles.css` is a published export and `sideEffects` lists `*.css` so bundlers keep it. ([validated by exports gains "./styles.css" alongside the unchanged "." entry](../tests/styles.test.ts#L107), [side-effects](../tests/styles.test.ts#L114))
3. **Type Definitions Included** — `.d.ts` files are bundled for full TypeScript IDE support. ([validated by npm pack --dry-run ships the five hooks and ErrorBoundary with their d.ts files](../tests/hooks-dist.test.ts#L23))
...
1. **`"use client"` Is Per-File** — The directive is added only to files triggering a client-only rule (a hook-shaped import, `createContext`, a `Component` subclass, a browser global, or an `on[A-Z]` JSX handler); 23 source files carry it and `src/index.ts` carries none. ([validated by exits zero against src/ and dist/](../tests/client-directives.test.ts#L90))
